...
osThreadId_t routine_thread_id;
const osThreadAttr_t routine_thread_wa = {
  .name = "routine_thread",
...
void encoder_update_config(volatile mc_configuration *conf) {
	...
	case SENSOR_PORT_MODE_ABI: {
		encoder_cfg_ABI.counts = conf->m_encoder_counts;
		//屏蔽__HAL_TIM_SET_AUTORELOAD(encoder_cfg_ABI.timer, encoder_cfg_ABI.counts - 1);
	} break;
...
//	if (m_encoder_type_now == ENCODER_TYPE_MT6816) {
//		enc_mt6816_deinit(&encoder_cfg_mt6816);
//	}
	m_encoder_type_now = ENCODER_TYPE_NONE;
}
...
//	commands_printf("Done!\n");
//}
static void routine_thread(void *argument){
	(void)argument;
...
//			break;
		...
		case routine_rate_1k: osDelayUs(TIMER_HZ / 1000,encoder_cc); break;
		case routine_rate_2k: osDelayUs(TIMER_HZ / 2000,encoder_cc); break;
		case routine_rate_5k: osDelayUs(TIMER_HZ / 5000,encoder_cc); break;
		case routine_rate_10k: osDelayUs(TIMER_HZ / 10000,encoder_cc); break;
		default: osDelayUs(1000,encoder_cc);
		}
	}
}
...
static void timer_start(routine_rate_t rate) {
	...
	if (!routine_running) {
		...
		routine_thread_id = osThreadNew(routine_thread, NULL, &routine_thread_wa);
