...
void conf_general_init(void) {
	...
	HAL_FLASH_Unlock();
	__HAL_FLASH_CLEAR_FLAG(FLASH_FLAG_OPERR | FLASH_FLAG_WRPERR | FLASH_FLAG_PGAERR |FLASH_FLAG_PGSERR);
	...
	for (unsigned int i = 0;i < (sizeof(backup_data) / 2);i++) {
		if (EE_ReadVariable16bits(EEPROM_BASE_BACKUP + i, &var) == EE_OK) {
...
__HAL_FLASH_CLEAR_FLAG(FLASH_FLAG_OPERR | FLASH_FLAG_WRPERR | FLASH_FLAG_PGAERR |FLASH_FLAG_PGSERR);
	...
	for (unsigned int i = 0;i < (sizeof(backup_data) / 2);i++) {
		...
		ee_status = EE_WriteVariable16bits(EEPROM_BASE_BACKUP + i, var);
		EE_ReadVariable16bits(EEPROM_BASE_BACKUP + i,&Back[i]);
		if(var != Back[i])
		{
			is_ok = false;
			break;
...
static bool read_eeprom_var(eeprom_var *v, int address, uint16_t base) {
	...
	if (EE_ReadVariable16bits(base + 2 * address, &var0) == EE_OK &&
			EE_ReadVariable16bits(base + 2 * address + 1, &var1) == EE_OK) {
...
//	EE_Init(EE_CONDITIONAL_ERASE);
	...
	ee_status = EE_WriteVariable16bits(base + address * 2, var0);
	if ((ee_status & EE_STATUSMASK_CLEANUP) == EE_STATUSMASK_CLEANUP) {
		ee_status|= EE_CleanUp();
	}
	...
	if (is_ok) {
		ee_status = EE_WriteVariable16bits(base + address * 2 + 1, var1);
		if ((ee_status & EE_STATUSMASK_CLEANUP) == EE_STATUSMASK_CLEANUP) {
			ee_status|= EE_CleanUp();
		}
...
void conf_general_read_mc_configuration(mc_configuration *conf, bool is_motor_2) {
	...
	for (unsigned int i = 1;i <= (sizeof(mc_configuration) / 2);i++) {
		if (EE_ReadVariable16bits(base + i, &var) == EE_OK) {
			conf_addr[2 * i] = (var >> 8) & 0xFF;
			conf_addr[2 * i + 1] = var & 0xFF;
		} else {
			is_ok = false;
...
//	timeout_configure_IWDT_slowest();
	...
	for (unsigned int i = 1;i <= (sizeof(mc_configuration) / 2);i++) {
		...
		ee_status = EE_WriteVariable16bits(base + i, var);
		if ((ee_status & EE_STATUSMASK_CLEANUP) == EE_STATUSMASK_CLEANUP) {
			ee_status|= EE_CleanUp();
		}
...
//				timeout_configure(tout, tout_c, tout_ksw);
		...
		if (diff > 2.0f && (diff_mid - diff / 2.0f) < (diff / 4)) {
			float offset, ratio;
			bool inverted;
...
//		timeout_configure(tout, tout_c, tout_ksw);
	...
	bool res;
	faultM1 = conf_general_measure_flux_linkage_openloop(i_max / 2.5f, 0.3f, 1800, r, l,
...
//		}
		...
		conf_general_store_mc_configuration(mcconf_old,false);//Ð´Èëflash
		mc_interface_set_configuration(mcconf_old);
