...
#define virtual_motor_is_connected() 	0
#define virtual_motor_get_angle_deg() 0 		
...
volatile motor_all_state_t m_motor_1;
...
static osThreadId_t pid_thread_id;
static const osThreadAttr_t pid_thread_attributes = {
  .name = "pid_thread",
  .priority = (osPriority_t) osPriorityNormal,
  .stack_size = 128 * 4
};

static osThreadId_t hfi_thread_id;
static const osThreadAttr_t hfi_threa_attributes = {
  .name = "hfi_thread",
  .priority = (osPriority_t) osPriorityNormal,
  .stack_size = 128 * 4
};
...
void mcpwm_foc_init(mc_configuration *conf_m1, mc_configuration *conf_m2) {
	...
	if (m_motor_1.m_conf->foc_offsets_cal_mode & (1 << 0)) {
		...
		if (!m_dccal_done) {
			...
			systime_t v_in_stable_time = osKernelGetTickCount();
			while (utils_age_s(v_in_stable_time) < 2.0) {
				...
				if (fabsf(v_in_now - v_in_last) > 1.5f) {
					v_in_last = v_in_now;
					v_in_stable_time = osKernelGetTickCount();
				}
...
float mcpwm_foc_get_sampling_frequency_now(void) {
	...
	if (get_motor_now()->m_conf->foc_control_sample_mode == FOC_CONTROL_SAMPLE_MODE_V0_V7) {
		...
		return get_motor_now()->m_conf->foc_f_zv / 2.0f;
	}
#else
	return get_motor_now()->m_conf->foc_f_zv / 2.0f;
#endif
...
//	timeout_configure(60000, 0.0, KILL_SW_MODE_DISABLED);
	...
	for (int i = 0; i < it_rat; i++) {
		...
		for (float j = phase_ovr_tmp; j < phase_ovr_tmp + (2.0f / 3.0f) * M_PI;
			 j += (2.0f * M_PI) / 500.0f) {
...
//		timeout_reset();
	...
	for (int i = 0; i < it_rat; i++) {
		float phase_old = motor->m_phase_now_encoder;
		float phase_ovr_tmp = motor->m_phase_now_override;
		for (float j = phase_ovr_tmp; j > phase_ovr_tmp - (2.0f / 3.0f) * M_PI; j -= (2.0f * M_PI) / 500.0f) {
			motor->m_phase_now_override = j;
			fault = mc_interface_get_fault();
			if (fault != FAULT_CODE_NONE) {
				goto exit_encoder_detect;
...
//		timeout_reset();
	...
	*ratio = roundf(((2.0f / 3.0f) * 180.0f) / fabsf(diff));
	...
	// Rotate
	for (float i = motor->m_phase_now_override;i < 2.0f * M_PI;i += (2.0f * M_PI) / 500.0f) {
		motor->m_phase_now_override = i;
		fault = mc_interface_get_fault();
		if (fault != FAULT_CODE_NONE) {
			goto exit_encoder_detect;
	...
	for (int i = 0;i < it_ofs;i++) {
		float step = (2.0f * M_PI * motor->m_conf->foc_encoder_ratio) / ((float)it_ofs);
...
void mcpwm_foc_adc_int_handler(void *p, uint32_t flags) {
	...
	if (conf_now->foc_control_sample_mode == FOC_CONTROL_SAMPLE_MODE_V0_V7) {
		dt = 1.0f / conf_now->foc_f_zv;
	} else {
		dt = 1.0f / (conf_now->foc_f_zv / 2.0f);
	}
#else
	float dt = 1.0f / (conf_now->foc_f_zv / 2.0f);
#endif
	...
	UTILS_LP_FAST(motor_now->m_motor_state.v_bus, GET_INPUT_VOLTAGE(), 0.1f);
	...
	if (motor_now->m_state == MC_STATE_RUNNING) {
		if (conf_now->foc_current_sample_mode == FOC_CURRENT_SAMPLE_MODE_ALL_SENSORS) {
			...
			motor_now->m_motor_state.i_alpha = (2.0f / 3.0f) * ia - (1.0f / 3.0f) * ib - (1.0f / 3.0f) * ic;
		...
		if (motor_now->m_i_alpha_beta_has_offset) {
			motor_now->m_motor_state.i_alpha = 0.5f * (motor_now->m_motor_state.i_alpha + motor_now->m_i_alpha_sample_next);
			motor_now->m_motor_state.i_beta = 0.5f * (motor_now->m_motor_state.i_beta + motor_now->m_i_beta_sample_next);
		...
		if (control_duty) {
			...
			if (fabsf(duty_set) < (duty_abs - 0.01f) &&
				...
				// Compute parameters
				float scale = 1.0f / motor_now->m_motor_state.v_bus;
...
//						utils_read_hall(motor_now != &m_motor_1, conf_now->m_hall_extra_samples));
			...
			case FOC_SENSOR_MODE_HFI_START:
				...
				if (motor_now->m_phase_observer_override) {
					...
					motor_now->m_motor_state.id_target = 0.0f;
			...
			if (motor_now->m_control_mode == CONTROL_MODE_HANDBRAKE) {
				...
				motor_now->m_motor_state.phase = 0.0f;
...
//		run_fw(motor_now, dt);
		...
		motor_now->m_motor_state.i_alpha = 0.0f;
		motor_now->m_motor_state.i_beta = 0.0f;
		motor_now->m_motor_state.id = 0.0f;
		motor_now->m_motor_state.iq = 0.0f;
		motor_now->m_motor_state.id_filter = 0.0f;
		motor_now->m_motor_state.iq_filter = 0.0f;
		motor_now->m_duty_i_term = 0.0f;
#ifdef HW_HAS_INPUT_CURRENT_SENSOR
		GET_INPUT_CURRENT_OFFSET(); // TODO: should this be done here?
#endif
		motor_now->m_motor_state.i_bus = 0.0f;
		motor_now->m_motor_state.i_abs = 0.0f;
		motor_now->m_motor_state.i_abs_filter = 0.0f;

		// Track back emf
		update_valpha_vbeta(motor_now, 0.0f, 0.0f);
...
//						utils_read_hall(motor_now != &m_motor_1, conf_now->m_hall_extra_samples));
			...
			case FOC_SENSOR_MODE_HFI_START:{
				motor_now->m_motor_state.phase = motor_now->m_phase_now_observer;
				if (fabsf(RADPS2RPM_f(motor_now->m_pll_speed)) < (conf_now->foc_sl_erpm_hfi * 1.1f)) {
					motor_now->m_hfi.est_done_cnt = 0;
					motor_now->m_hfi.flip_cnt = 0;
				}
			} break;
		...
		UTILS_LP_FAST(motor_now->m_motor_state.vd, vd_tmp, 0.2f);
		UTILS_LP_FAST(motor_now->m_motor_state.vq, vq_tmp, 0.2f);
		...
		const float voltage_normalize = 1.5f / motor_now->m_motor_state.v_bus;
		...
		UTILS_LP_FAST(motor_now->m_motor_state.mod_q_filter, motor_now->m_motor_state.mod_q, 0.2f);
		utils_truncate_number_abs((float*)&motor_now->m_motor_state.mod_q_filter, 1.0f);
	}
	...
	// Low latency speed estimation, for e.g. HFI and speed control.
	{
		...
		utils_truncate_number(&diff, -M_PI / 3.0f, M_PI / 3.0f);

		UTILS_LP_FAST(motor_now->m_speed_est_fast, diff / dt, 0.01f);
		UTILS_NAN_ZERO(motor_now->m_speed_est_fast);

		UTILS_LP_FAST(motor_now->m_speed_est_faster, diff / dt, 0.2f);
		...
		utils_truncate_number(&diff_corr, -M_PI / 3.0f, M_PI / 3.0f);

		UTILS_LP_FAST(motor_now->m_speed_est_fast_corrected, diff_corr / dt, 0.01f);
	...
	if (conf_now->p_pid_ang_div > 0.98f && conf_now->p_pid_ang_div < 1.02f) {
		...
		if (angle_now < 90.0f && motor_now->m_pid_div_angle_last > 270.0f) {
			motor_now->m_pid_div_angle_accumulator += 360.0f / conf_now->p_pid_ang_div;
...
static void timer_update(motor_all_state_t *motor, float dt) {
	...
	if (motor->m_min_rpm_timer > 0.0f) {
		...
		if (started_now) {
			if (motor->m_motor_state.duty_now > 0.0f) {
				motor->m_phase_now_observer_override += M_PI / 3.0f;
			} else {
				motor->m_phase_now_observer_override -= M_PI / 3.0f;
			}
		}
		...
		float s, c;
		utils_fast_sincos_better(motor->m_phase_now_observer_override + SIGN(motor->m_motor_state.duty_now) * M_PI / 4.0f, &s, &c);
		...
		if ((motor->m_conf->foc_sensor_mode == FOC_SENSOR_MODE_HFI_V4 ||
			...
			if (motor->m_conf->foc_hfi_amb_mode == FOC_AMB_MODE_SIX_VECTOR || est_done) {
				...
				if (motor->m_hfi.est_done_cnt < motor->m_conf->foc_hfi_start_samples) {
					...
					if (fabsf(utils_angle_difference_rad(angle_bin_2, angle_bin_1)) > (M_PI / 2.0f)) {
...
//		case PID_RATE_25_HZ: osDelayUS(1000000 / 25,2); break;
//		case PID_RATE_50_HZ: osDelayUS(1000000 / 50,2); break;
//		case PID_RATE_100_HZ: osDelayUS(1000000 / 100,2); break;
//		case PID_RATE_250_HZ: osDelayUS(1000000 / 250,2); break;
//		case PID_RATE_500_HZ: osDelayUS(1000000 / 500,2); break;
//		case PID_RATE_1000_HZ: osDelayUS(1000000 / 1000,2); break;
//		case PID_RATE_2500_HZ: osDelayUS(1000000 / 2500,2); break;
//		case PID_RATE_5000_HZ: osDelayUS(1000000 / 5000,2); break;
//		case PID_RATE_10000_HZ: osDelayUS(1000000 / 10000,2); break;
//		}
		osDelay(1);
//		osDelayUS(1000,2);
...
static void control_current(motor_all_state_t *motor, float dt) {
	...
	if (conf_now->foc_d_gain_scale_start < 0.99f) {
		float max_mod_norm = fabsf(state_m->duty_now / max_duty);
		if (max_duty < 0.01f) {
			max_mod_norm = 1.0f;
		}
		if (max_mod_norm > conf_now->foc_d_gain_scale_start) {
			d_gain_scale = utils_map(max_mod_norm, conf_now->foc_d_gain_scale_start, 1.0f,
					1.0f, conf_now->foc_d_gain_scale_max_mod);
	...
	//update_valpha_vbeta(motor, state_m->mod_alpha_raw, state_m->mod_beta_raw);使用电流环输出的Ud,Uq

	motor->m_motor_state.v_alpha = state_m->mod_alpha_raw / voltage_normalize;
	motor->m_motor_state.v_beta  = state_m->mod_beta_raw / voltage_normalize;
