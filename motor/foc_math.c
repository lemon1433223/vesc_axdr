...
void foc_run_pid_control_speed(bool index_found, float dt, motor_all_state_t *motor) {
	...
	utils_truncate_number_abs(&motor->m_speed_i_term, 0.5f);
...
float foc_correct_hall(float angle, float dt, motor_all_state_t *motor, int hall_val) {
	...
	float rad_per_sec_hall = (M_PI / 3.0f) / motor->m_hall_dt_diff_last;
	...
	if (ang_hall_int < 201) {
		...
		float ang_hall_now = ((float)ang_hall_int / 200.0f) * 2.0f * M_PI;
		...
		if (motor->m_ang_hall_int_prev < 0) {
			...
			// Scale to the circle and convert to radians
			motor->m_ang_hall = ((float)ang_avg / 200.0f) * 2.0f * M_PI;
		}

		motor->m_ang_hall_int_prev = ang_hall_int;

		if (RADPS2RPM_f((M_PI / 3.0f) / fmaxf(fabsf(motor->m_hall_dt_diff_now),
			...
			if (fabsf(diff) < ((2.0f * M_PI) / 12.0f) || SIGN(diff) != SIGN(rad_per_sec_hall)) {
				// Do interpolation
		...
		// changes fast.
		float angle_step = (fmaxf(rpm_abs_hall, conf_now->foc_hall_interp_erpm) / 60.0f) * 2.0f * M_PI * dt * 1.5f;
...
void foc_run_fw(motor_all_state_t *motor, float dt) {
	...
	if (motor->m_state == MC_STATE_RUNNING &&
		...
		if (motor->m_conf->foc_fw_duty_start < 0.99f &&
			...
			fw_current_now = utils_map(duty_abs,
					...
					0.0f, motor->m_conf->foc_fw_current_max);
...
void foc_precalc_values(motor_all_state_t *motor) {
	...
	motor->p_v2_v3_inv_avg_half = (0.5f / motor->p_lq + 0.5f / motor->p_ld) * 0.9f; // With the 0.9 we undo the adjustment from the detection
