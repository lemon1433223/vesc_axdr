...
// Private variables
volatile motor_if_state_t m_if_motor_1;
#ifdef HW_HAS_DUAL_MOTORS
static volatile motor_if_state_t m_if_motor_2;
#endif
...
static const osThreadAttr_t timer_thread_attributes = {
  .name = "mcif_timer_thread",
  .priority = (osPriority_t) osPriorityNormal,
  .stack_size = 256 * 4
};

static osThreadId_t sample_send_thread_id;
static const osThreadAttr_t sample_send_thread_attributes = {
  .name = "SampleSender",
  .priority = (osPriority_t) osPriorityNormal,
  .stack_size = 128 * 4
};

static osThreadId_t fault_stop_thread_id;
static const osThreadAttr_t fault_stop_thread_attributes = {
  .name = "fault_stop",
  .priority = (osPriority_t) osPriorityNormal,
  .stack_size = 128 * 4
};

static osThreadId_t stat_thread_id;
static const osThreadAttr_t stat_thread_attributes = {
  .name = "StatCounter",
  .priority = (osPriority_t) osPriorityNormal,
  .stack_size = 128 * 4
};
...
void mc_interface_init(void) {
	memset((void*)&m_if_motor_1, 0, sizeof(motor_if_state_t));
#ifdef HW_HAS_DUAL_MOTORS
	memset((void*)&m_if_motor_2, 0, sizeof(motor_if_state_t));
#endif

	conf_general_read_mc_configuration((mc_configuration*)&m_if_motor_1.m_conf, false);
#ifdef HW_HAS_DUAL_MOTORS
	conf_general_read_mc_configuration((mc_configuration*)&m_motor_2.m_conf, true);
#endif

	/*ÐÞ¸ÄÄÚÈÝ*/
//	confgenerator_set_defaults_mcconf((mc_configuration*)&m_if_motor_1.m_conf);
	/*ÐÞ¸ÄÄÚÈÝ*/
#ifdef HW_HAS_DUAL_MOTORS
	m_if_motor_1.m_conf.motor_type = MOTOR_TYPE_FOC;
	m_if_motor_2.m_conf.motor_type = MOTOR_TYPE_FOC;
#endif
...
//	chThdCreateStatic(stat_thread_wa, sizeof(stat_thread_wa), NORMALPRIO, stat_thread, NULL);
  ...
  timer_thread_id = osThreadNew(timer_thread, NULL, &timer_thread_attributes);
	sample_send_thread_id = osThreadNew(sample_send_thread, NULL, &sample_send_thread_attributes);
	fault_stop_thread_id = osThreadNew(fault_stop_thread, NULL, &fault_stop_thread_attributes);
	stat_thread_id = osThreadNew(stat_thread, NULL, &stat_thread_attributes);
	...
	case MOTOR_TYPE_FOC:
#ifdef HW_HAS_DUAL_MOTORS
		mcpwm_foc_init((mc_configuration*)&m_if_motor_1.m_conf, (mc_configuration*)&m_if_motor_2.m_conf);
#else
		mcpwm_foc_init((mc_configuration*)&m_if_motor_1.m_conf, (mc_configuration*)&m_if_motor_1.m_conf);
#endif
		break;
	...
	bms_init((bms_config*)&m_if_motor_1.m_conf.bms);
}
...
int mc_interface_get_motor_thread(void) {
	return 1;//ÆÁ±ÎchThdGetSelfX()->motor_selected;
}
...
//	update_override_limits(motor, &motor->m_conf);
	...
	case MOTOR_TYPE_FOC:
#ifdef HW_HAS_DUAL_MOTORS
		if (motor == &m_if_motor_1) {
			m_if_motor_2.m_conf.foc_f_zv = motor->m_conf.foc_f_zv;
			m_if_motor_2.m_conf.motor_type = motor->m_conf.motor_type;
		} else {
			m_if_motor_1.m_conf.foc_f_zv = motor->m_conf.foc_f_zv;
			m_if_motor_1.m_conf.motor_type = motor->m_conf.motor_type;
		}
#endif
	...
	//ÆÁ±Îbms_init(&configuration->bms);
}
...
void mc_interface_sample_print_data(debug_sampling_mode mode, uint16_t len, uint8_t decimation, bool raw, 
	...
	if (mode == DEBUG_SAMPLING_SEND_LAST_SAMPLES) {
		//ÆÁ±ÎchEvtSignal(sample_send_tp, (eventmask_t) 1);
		...
		m_sample_raw = raw;
#ifdef HW_HAS_DUAL_MOTORS
		m_sample_is_second_motor = motor_now() == &m_if_motor_2;
#endif
	}
}
...
float mc_interface_get_battery_level(float *wh_left) {
	...
	case BATTERY_TYPE_LIION_3_0__4_2:
		...
		ah_tot *= 0.85f; // 0.85 because the battery is not fully depleted at 3.2V / cell
...
float mc_interface_get_speed(void) {
	...
	return (rpm / 60.0f) * conf->si_wheel_diameter * M_PI / conf->si_gear_ratio;
#endif
}
...
void mc_interface_ignore_input_both(int time_ms) {
	m_if_motor_1.m_ignore_iterations = time_ms;

#ifdef HW_HAS_DUAL_MOTORS
	m_if_motor_2.m_ignore_iterations = time_ms;
#endif
}
...
void mc_interface_fault_stop(mc_fault_code fault, bool is_second_motor, bool is_isr) {
	...
	osKernelLock();
	osThreadFlagsSet(fault_stop_thread_id,0x01);
	osKernelUnlock();
}
...
//ÆÁ±Î	ledpwm_update_pwm();

#ifdef HW_HAS_DUAL_MOTORS
	motor_if_state_t *motor = is_second_motor ? (motor_if_state_t*)&m_if_motor_2 : (motor_if_state_t*)&m_if_motor_1;
#else
	motor_if_state_t *motor = (motor_if_state_t*)&m_if_motor_1;
	...
	float t_samp = 1.0f / motor->m_f_samp_now;
	...
	case DEBUG_SAMPLING_NOW:
		if (m_sample_now == m_sample_len) {
			...
			osThreadFlagsSet(sample_send_thread_id,0x01);
		} else {
			sample = true;
		}
		break;
	...
	if (sample) {
		...
		if (a >= m_sample_int) {
			...
			m_f_sw_samples[m_sample_now] = (int16_t)(0.1f / t_samp / m_sample_int);
...
void mc_interface_adc_inj_int_handler(void) {
	switch (m_if_motor_1.m_conf.motor_type) {
	case MOTOR_TYPE_BLDC:
	case MOTOR_TYPE_DC:
 ...
 */
static void update_override_limits(volatile motor_if_state_t *motor, volatile mc_configuration *conf) {
	bool is_motor_1 = motor == &m_if_motor_1;
	...
	if (v_in > (conf->l_battery_cut_start - 0.1f)) {
		lo_in_max_batt = conf->l_in_current_max;
	} else if (v_in < (conf->l_battery_cut_end + 0.1f)) {
		lo_in_max_batt = 0.0f;
	} else {
	...
	float lo_in_min_batt = 0.0f;
	if (v_in < (conf->l_battery_regen_cut_start + 0.1f)) {
		lo_in_min_batt = conf->l_in_current_min;
	} else if (v_in > (conf->l_battery_regen_cut_end - 0.1f)) {
		lo_in_min_batt = 0.0f;
	} else {
		lo_in_min_batt = utils_map(v_in, conf->l_battery_regen_cut_start,
				conf->l_battery_regen_cut_end, conf->l_in_current_min, 0.0f);
	}
	...
	// BMS limits
	//ÆÁ±Îbms_update_limits(&lo_in_min,  &lo_in_max, conf->l_in_current_min, conf->l_in_current_max);
...
static volatile motor_if_state_t *motor_now(void) {
#ifdef HW_HAS_DUAL_MOTORS
	return mc_interface_motor_now() == 1 ? &m_if_motor_1 : &m_if_motor_2;
#else
	return &m_if_motor_1;
#endif
}
...
static void run_timer_tasks(volatile motor_if_state_t *motor) {
	...
	if (is_motor_1) {
		...
		g_backup.odometer += odometer - m_if_motor_1.m_odometer_last;
		m_if_motor_1.m_odometer_last = odometer;

		uint64_t runtime = osKernelGetTickCount() / osKernelGetTickFreq();

		// Handle wrap around
		if (runtime < m_if_motor_1.m_runtime_last) {
			m_if_motor_1.m_runtime_last = 0;
		}

		g_backup.runtime += runtime - m_if_motor_1.m_runtime_last;
		m_if_motor_1.m_runtime_last = runtime;
	}
...
#ifdef HW_HAS_DUAL_MOTORS
		mcpwm_foc_get_current_offsets(&curr0_offset, &curr1_offset, &curr2_offset, motor == &m_if_motor_2);
#else
...
static void timer_thread(void * argument) {
	...
	for(;;) {
		run_timer_tasks(&m_if_motor_1);
#ifdef HW_HAS_DUAL_MOTORS
		run_timer_tasks(&m_if_motor_2);
#endif
...
static void update_stats(volatile motor_if_state_t *motor) {
	mc_interface_select_motor_thread(motor == (&m_if_motor_1) ? 1 : 2);
...
void mc_interface_stat_reset(void) {
	...
	s->time_start = osKernelGetTickCount();
...
static void stat_thread(void * argument){
	...
	for(;;) {
//		update_stats(&m_if_motor_1);
#ifdef HW_HAS_DUAL_MOTORS
		update_stats(&m_if_motor_2);
#endif
...
#ifdef HW_HAS_DUAL_MOTORS
		volatile motor_if_state_t *motor = fault_data_copy.is_second_motor ? &m_if_motor_2 : &m_if_motor_1;
#else
		volatile motor_if_state_t *motor = &m_if_motor_1;
#endif
...
unsigned mc_interface_calc_crc(mc_configuration* conf_in, bool is_motor_2) {
	...
	if (conf == NULL) {
		if(is_motor_2) {
#ifdef HW_HAS_DUAL_MOTORS
			conf = &(m_if_motor_2.m_conf);
#else
			return 0; //shouldn't be here
#endif
		} else {
			conf = &(m_if_motor_1.m_conf);
		}
	}
