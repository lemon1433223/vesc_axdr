...
//		commands_printf(" ");
		...
		if (argc == 4) {
			...
			if (current > 0.0f && current < mc_interface_get_configuration()->l_current_max &&
				...
				int8_t hall_table[8];
				int hall_res;
				//conf_general_detect_motor_param(current, min_rpm, low_duty, &cycle_integrator, &coupling_k, hall_table, &hall_res)
				if (0) {
...
//				commands_send_packet_nrf(buffer, 2);
		...
		if (argc == 2) {
			...
			if (current > 0.0f && current <= mc_interface_get_configuration()->l_current_max) {
				int res;
				int fault = 0;//conf_general_autodetect_apply_sensors_foc(current, true, true, &res);
...
//						timeout_reset();
		...
		if (argc == 2) {
			...
			if (max_power_loss > 0.0f) {
				...
				int res = 0;//ÆÁ±Îconf_general_detect_apply_all_foc_can(true, max_power_loss, 0.0, 0.0, 0.0, 0.0, NULL);
		...
		commands_printf("Uptime: %.2f s\n", (float)(osKernelGetTickCount()/osKernelGetTickFreq()));
...
//ÆÁ±Î		unsigned mc_crc0 = mc_interface_get_configuration()->crc;
...
//ÆÁ±Î		commands_printf("Git Branch: %s", GIT_BRANCH_NAME);
