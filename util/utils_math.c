...
float utils_interpolate_angles_rad(float a1, float a2, float weight_a1) {
	while ((a1 - a2) > M_PI) a2 += 2.0f * M_PI;
	while ((a2 - a1) > M_PI) a1 += 2.0f * M_PI;
...
{
    // Normalize the angle x to the range [-pi, pi] for better accuracy
    float angle = fmodf(x, 2 * M_PI);

    // Use the sin and cos algorithms to calculate both values
    *sin_val = sinf(angle);
    *cos_val = cosf(angle);
}
