...
static osMutexId_t packet_buffer_mutex;
const osMutexAttr_t packet_buffer_mutex_attributes = {
  .name = "packet_buffer_mutex"
};
void mempools_init(void) {
	packet_buffer_mutex = osMutexNew(&packet_buffer_mutex_attributes);
}
...
uint8_t *mempools_get_packet_buffer(void) {
	osMutexAcquire(packet_buffer_mutex,osWaitForever);
	return packet_buffer;
}

void mempools_free_packet_buffer(uint8_t *buffer) {
	if (buffer == packet_buffer) {
		osMutexRelease(packet_buffer_mutex);
	}
}
